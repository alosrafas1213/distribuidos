...
int main()
{
	...
	do
	{
		...
		sock_dialog=accept(sock_escucha,(struct sockaddr *)&dir_cli,(socklen_t*)&length);
...
char *respuesta[] = {okey1,finn1};		//respuestas del servidor
	...
	write(s,buf,strlen(buf));
	printf("\nIP=%ld       Conectado\n",(long)ntohl(info->sin_addr.s_addr));
	...
	do
	{
		...
		switch(parser_comandos(buf))
		{
			case 1:	// SYNC
				...
				printf("%s: seg:%ld\0",respuesta[0],horaservidor.tv_sec);
				lon=strlen(buf);
				limpiar_buffer(buf,lon+1,29);
				sprintf(&buf[30],"useg:%ld\0",horaservidor.tv_usec);
				printf("\nuseg:%ld\0",horaservidor.tv_usec);
				printf("\n%s",&buf[30]);
				lon2=strlen(&buf[30]);
				write(s,buf,30+lon2);
